...
How often that happens depends on who wrote the scene, and the difference is larger than
expected. Measured in game:

| | shapes | meshes |
|---|---|---|
| `examples/12-click.lua`, placed by hand | 19 | 1 |
| a page generated by the HTML mod | 48 | 13 |

A person placing rows, pills and accent bars puts them where they do not collide, so nothing
cuts. A layout engine emitting boxes does not, so a fair number do. **Neither is worth worrying
about**: on the 48-shape page the whole difference was 0.06 → 0.13 ms of mesh upload, against a
16.7 ms frame, and tessellation is unchanged because it is the same geometry handed over in more
pieces.
